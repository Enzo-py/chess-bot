...
with open('data/stockfish_eval_results.json', 'r') as f:
    results = json.load(f)
...
for game in results:
    ...
    metrics[white_ai]['total_blunders'] += sum(1 for quality, count in game_metrics['white']['move_qualities'].items() if quality == 'blunder')
    metrics[white_ai]['total_excellent_moves'] += sum(1 for quality, count in game_metrics['white']['move_qualities'].items() if quality in ['brilliant', 'excellent'])
    ...
    metrics[black_ai]['total_blunders'] += sum(1 for quality, count in game_metrics['black']['move_qualities'].items() if quality == 'blunder')
    metrics[black_ai]['total_excellent_moves'] += sum(1 for quality, count in game_metrics['black']['move_qualities'].items() if quality in ['brilliant', 'excellent'])
...
plt.subplot(2, 2, 1)
bars = plt.bar(ai_names, avg_acpl)
...
# 2. Blunders per Game
plt.subplot(2, 2, 2)
bars = plt.bar(ai_names, blunders_per_game)
plt.title('Blunders per Game by AI')
plt.xticks(rotation=45, ha='right')
...
plt.subplot(2, 2, 3)
bars = plt.bar(ai_names, excellent_moves_percentage)
...
plt.subplot(2, 2, 4)
bars = plt.bar(ai_names, moves_per_game)
